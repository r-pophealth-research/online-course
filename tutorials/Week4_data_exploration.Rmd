...
`read_csv()` has a bunch of options that let us read in different files, but the only required input is the `file` we want in R. We give the file we want to import using a character—in this case, the URL of the CSV on GitHub:
...
Notice that when we imported the wage data above, the data were printed below the code cell but we didn't save them to a variable. That's because we didn't use assignment! To use imported data in later code cells, assign it to a name. Try importing the wage data again, this time assigning it to a variable called `wages`.
