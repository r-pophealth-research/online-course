...
Earlier chapters helped you understand the basics of R, import your data, and create the variables you need to do your analyses. In the remaining chapters, we will focus on how you can use R in your statistical analyses. The first step in any research project is to understand the data set you are working with. We will use the `framingham.csv` dataset from the [course GitHub repo](https://github.com/r-pophealth-research/online-course/tree/main/data).

```{r import}
fram_data = read_csv("https://raw.githubusercontent.com/r-pophealth-research/online-course/main/data/framingham.csv")
head(fram_data)
```
...
# Try to make the output of table() into a data frame using data.frame() and pivot_wider()
...
```{r}
clean_fram_data <- clean_fram_data %>%
  mutate(smoker = if_else(currentSmoker == 1, "Smoker", "No smoking"))

table(clean_fram_data$smoker, useNA="ifany")
```
:::
...
::: {.callout-tip collapse="true" title="Solution"}
```{r}
prop.table(table(clean_fram_data$sex, clean_fram_data$educ_labeled))["Female", "High school diploma/GED"]

prop.table(table(clean_fram_data$sex, clean_fram_data$educ_labeled), margin=1)["Male", "4-year degree or higher"]

prop.table(table(clean_fram_data$sex, clean_fram_data$educ_labeled), margin=2)["Female", "4-year degree or higher"]
```
:::
...
::: {.callout-tip collapse="true" title="Solution"}
```{r}
risk_chd_bpmeds <- clean_fram_data %>%
  filter(BPMeds == 1) %>%
  pull(TenYearCHD) %>%
  mean()

risk_chd_no_bpmeds <- clean_fram_data %>%
  filter(BPMeds == 0) %>%
  pull(TenYearCHD) %>%
  mean()

risk_chd_bpmeds/risk_chd_no_bpmeds
```
:::
...
::: {.callout-tip collapse="true" title="Solution"}
```{r}
odds_chd_bpmeds_num <- clean_fram_data %>%
  filter(BPMeds == 1, TenYearCHD == 1) %>% nrow()
odds_chd_bpmeds_den <- clean_fram_data %>%
  filter(BPMeds == 1, TenYearCHD == 0) %>% nrow()
odds_chd_bpmeds <- odds_chd_bpmeds_num/odds_chd_bpmeds_den

odds_chd_no_bpmeds_num <- clean_fram_data %>%
  filter(BPMeds == 0, TenYearCHD == 1) %>% nrow()
odds_chd_no_bpmeds_den <- clean_fram_data %>%
  filter(BPMeds == 0, TenYearCHD == 0) %>% nrow()
odds_chd_no_bpmeds <- odds_chd_no_bpmeds_num/odds_chd_no_bpmeds_den

odds_chd_bpmeds/odds_chd_no_bpmeds
```
:::
...
mean(clean_fram_data$age)
sd(clean_fram_data$age)
mean(clean_fram_data$age[clean_fram_data$sex == "Female"])
sd(clean_fram_data$age[clean_fram_data$sex == "Female"])
mean(clean_fram_data$age[clean_fram_data$sex == "Male"])
sd(clean_fram_data$age[clean_fram_data$sex == "Male"])

# Smoking status counts and column percents
table(clean_fram_data$smoker, clean_fram_data$sex)
prop.table(table(clean_fram_data$smoker, clean_fram_data$sex), margin=2) * 100

# Education counts and column percents
table(clean_fram_data$educ_labeled, clean_fram_data$sex, useNA="ifany")
prop.table(table(clean_fram_data$educ_labeled, clean_fram_data$sex, useNA="ifany"), margin=2) * 100
```
:::
...
```{r}
library(table1)
table1(~ age + factor(smoker) + factor(educ_labeled) + factor(TenYearCHD) | factor(BPMeds),
       data = clean_fram_data %>% filter(!is.na(BPMeds)))
```
:::
