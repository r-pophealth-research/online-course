...
Run the cell below to load the `framingham.csv` dataset as `fram_data`.
...
Lastly, it is important to note that the outputs of `table()` are not data frames, so we cannot use dplyr pipes `%>%` to make changes to this table. 
...
Add a `smoker` column to `clean_fram_data` based on the `currentSmoker` column using the table above and then check your work using `table()`. *Challenge:* Try doing this using both `if_else()`.
