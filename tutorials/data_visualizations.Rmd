...
In this chapter, we'll revisit data visualizations and learn how to use the `ggplot2` package to create publication-ready figures. This tutorial is on the longer side—take your time working through it section by section.

# Loading Packages and Data

`ggplot2` is included as a core package of `tidyverse`, which is already loaded on this webpage. Why is is called `ggplot`? Because it is the Grammar of Graphics for PLOTting!

In this chapter, we will be using maternal mortality data from the World Health Organization that has been merged with population data from the World Bank. The data file is available in the [course GitHub repo](https://github.com/r-pophealth-research/online-course/tree/main/data).
...
maternal_mortality = read_csv("https://raw.githubusercontent.com/r-pophealth-research/online-course/main/data/maternal_mortality_ratios.csv")
...
|----------------------|-----------|----------------------------------------------------------------------------------------------|
| `Year`               | numeric   | Year of estimate                                                                             |
| `Country`            | character | Country of estimate                                                                          |
| `Region`             | character | The WHO region that the country belongs to                                                   |
| `income_group`       | character | The World Bank's classification of the country's income level                                |
| `total_population`   | numeric   | The population size in hundreds of thousands                                                 |
| `mortality_estimate` | numeric   | The estimated maternal mortality ratio, calculated as (# maternal deaths / # live births) x 100,000 |
| `mortality_ci_lb`    | numeric   | The lower bound of the estimated confidence interval of maternal mortality ratio             |
| `mortality_ci_ub`    | numeric   | The upper bound of the estimated confidence interval of maternal mortality ratio             |
...
# Potting bivariate distributions
...
We can also vary the shape of points and type of lines in our plots using `shape` and `linetype`, respectively.
...
The possible values for `shape` are numerics that correspond to specific shapes such as circles, squares, and stars. See this [resource](http://www.sthda.com/english/wiki/ggplot2-point-shapes) for a visual guide of what each shape looks like.
...
```{r}
yemen_oman_data = maternal_mortality %>% filter(Country %in% c("Yemen", "Oman"))

ggplot(yemen_oman_data, aes(x = Year, y = mortality_estimate,
