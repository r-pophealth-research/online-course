...
If we want to view specific observations (rows), we can use the `[]` operator (see the Data Structures chapter for a refresher).
...
In Data Structures, we learned how to use `[]` to select rows that meet a certain condition. For example, the code below shows only rows for study participants that were over the age of 65.
...
Next, we'll learn about data manipulation which includes methods to create new variables, combine datasets, and reshape data frames.
